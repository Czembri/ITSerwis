...
namespace ItSerwis_Merge_v2
{
    ...
    public partial class ShortServiceDocument : Window, IShortServiceDocument
    {
        ...
        public ShortServiceDocument(IUserValidation userValidation, 
        ...
        private void FillEmployeeData()
        {
            ...
            var user = _userValidation.GetUserCredentials();
            ...
            if (docID != 0)
            {
                var docServ = _serviceDocumentsAndDataSets.GetServiceDocumentFromDatabase(docID);
        ...
        private void Update(object sender, EventArgs e)
        {
            ...
            try
            {
                _serviceDocumentsAndDataSets.UpdateServiceDocument(docID, name.Text, lastname.Text, address.Text, user.firstname, user.lastname, userId, type.Text, brand.Text, model.Text, description.Text);
            }
            catch (Exception err)
            {
        ...
        private void InsertData(string now, string customerName, string customerLastName, string customerAddr, string employeeName, string employeeLastName, int parsedEmpNum, string deviceType, string deviceBrand, string deviceModel, string descr, string documentnumber)
        {
            try
            {
                _serviceDocumentsAndDataSets.InsertIntoServiceDocuments(now, customerName, customerLastName, customerAddr, employeeName, employeeLastName, parsedEmpNum, deviceType, deviceBrand, deviceModel, descr, documentnumber);
        ...
        private void InsertIntoClients(object sender, EventArgs e)
        {
            ...
            try
            {
                _serviceDocumentsAndDataSets.InsertIntoClientsFromServiceDocs(customerName, customerLastName, customerAddr, date, parsedDocumentID);
        ...
        {
            var lastDocId = _serviceDocumentsAndDataSets.GetLastDocumentID();
